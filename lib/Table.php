...
class Table
{
    ...
    public function get_relationship($name, $strict = false)
    {
        ...
        if ($strict) {
            throw new RelationshipException(
                "Relationship named $name has not been declared for class: {$this->class->getName()}"
            );
        }
    ...
    {
        if (
            ($pk = $this->class->getStaticPropertyValue('pk', null)) ||
    ...
    {
        if (
            ($table = $this->class->getStaticPropertyValue('table', null)) ||
        ...
        if (
            ($db = $this->class->getStaticPropertyValue('db', null)) ||
    ...
    private function set_setters_and_getters()
    {
        ...
        if (!empty($getters) || !empty($setters)) {
            trigger_error(
                ...
                http://www.phpactiverecord.org/projects/main/wiki/Utilities#attribute-getters
