...
class Validations
{
    ...
    public function validates_presence_of($attrs)
    {
        $configuration = array_merge(
            ...
            ['message' => Errors::$DEFAULT_ERROR_MESSAGES['blank'], 'on' => 'save']
        );
    ...
    public function validates_inclusion_or_exclusion_of($type, $attrs)
    {
        $configuration = array_merge(
            self::$DEFAULT_VALIDATION_OPTIONS,
            ['message' => Errors::$DEFAULT_ERROR_MESSAGES[$type], 'on' => 'save']
        );
    ...
    public function validates_format_of($attrs)
    {
        $configuration = array_merge(
            self::$DEFAULT_VALIDATION_OPTIONS,
            ['message' => Errors::$DEFAULT_ERROR_MESSAGES['invalid'], 'on' => 'save', 'with' => null]
        );
        ...
        foreach ($attrs as $attr) {
            ...
            if (is_null($options['with']) || !is_string($options['with'])) {
                throw new ValidationsArgumentError(
                    'A regular expression must be supplied
                    as the [with] option of the configuration array.'
                );
            } else {
    ...
    public function validates_length_of($attrs)
    {
        ...
        foreach ($attrs as $attr) {
            ...
            switch (sizeof($range_options)) {
                case 0:
                    throw new  ValidationsArgumentError(
                        'Range unspecified.  Specify the [within], [maximum], or [is] option.'
                    );
            ...
            if ($range_options[0] == 'within' || $range_options[0] == 'in') {
                ...
                if (!(Utils::is_a('range', $range))) {
                    throw new  ValidationsArgumentError(
                        "$range_options[0] must be an array composing a range of numbers with key [0] " .
                        "being less than key [1]"
                    );
                }
